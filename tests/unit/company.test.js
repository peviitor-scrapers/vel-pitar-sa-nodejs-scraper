...
const VEL_PITAR_ANAF_RECORD = {
  cui: 21229091,
  name: 'VEL PITAR SA',
  address: 'JUD. VÂLCEA, MUN. RÂMNICU VÂLCEA, STR. TIMIS, NR.22, ET.P',
  caenCode: '1071',
  registrationNumber: 'J2007000221389',
  inactive: false,
  vatRegistered: true,
  eFacturaRegistered: false,
  onrcStatusLabel: 'Funcțiune',
  headquartersAddress: { locality: 'Mun. Râmnicu Vâlcea' }
};
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch VEL PITAR via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(VEL_PITAR_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'VEL PITAR SA');
      expect(result).toHaveProperty('cif', '21229091');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('VEL PITAR SA');
    });
    ...
    it('should fall back to company config when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 21229091, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('21229091');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(VEL_PITAR_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'VEL PITAR SA' }]));
    ...
    // VEL PITAR e activă — testul inactive se rulează doar dacă firma e inactivă
    if (VEL_PITAR_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...VEL_PITAR_ANAF_RECORD, inactive: true };
